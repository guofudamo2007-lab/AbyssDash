...
### v1.3.0 - 进化深渊 (Current)
- **新增**: 完整实装 Roguelite 基因突变系统，提供三选一升级面板与 6 种进阶能力。
...
### v1.2.0 - 王者之战
- **新增**: 引入三阶段史诗级 Boss 战系统（深海巨型章鱼、幽灵潜艇、远古巨齿鲨）。
- **优化**: 优化 Boss 弹幕轨迹算法。
- **新增**: Boss 战独立 BGM 与警报系统。

### v1.1.0 - 艺术与听觉
- **新增**: 纯数学实时渲染的皮肤系统（Neo Vector, Golden Scale）。
- **新增**: 基于 Web Audio API 的纯代码演算生成音效 (Procedural Audio)。
- **优化**: 改进水雷和水母的碰撞判定，增加微弱吸附引力算法 (Boids)。

### v1.0.0 - 深海初探
- **核心**: 确立深海跑酷核心 Gameplay Loop（吞噬、冲刺、躲避）。
- **引擎**: 手写 2D Vanilla JS 游戏引擎，物理阻尼系统与流体浮力。
- **UI**: 实装 Glassmorphism 游戏主界面与成就图鉴系统。
