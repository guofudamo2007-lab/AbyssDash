...
assert.match(html, /<title>深渊冲刺 \(Abyss Dash\) v9\.9\.0<\/title>/);
assert.match(html, /<meta name="application-version" content="9\.9\.0">/);
assert.match(html, /id="version-label">v9\.9\.0 · ABYSS VOYAGE<\/div>/);
assert.match(readme, /version-v9\.9\.0-00d7ff/);
assert.match(readme, /当前版本：\*\*v9\.9\.0「深渊长航」\*\*/);
assert.match(readme, /### v9\.9\.0 — 深渊长航（2026-08-01，当前版本）/);
